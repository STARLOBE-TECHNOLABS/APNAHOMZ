...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!email) {
      notify({ content: "Email is required", type: "warning" });
      return;
    }
    ...
    try {
      ...
      console.log(data.message); // Log for easy access to link
      notify({ content: data.message, type: "success" });
    } catch (error) {
      notify({ content: error.message || "Network error", type: "error" });
    } finally {
      setLoading(false);
    }
  };
